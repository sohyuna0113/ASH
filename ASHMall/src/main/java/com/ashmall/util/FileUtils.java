...
public class FileUtils {
	...
	public static ResponseEntity<byte[]> getFile(String uploadPath, String fileName) throws Exception {
		...
		// 입력스트림 객체 : 자원(파일)을 대상으로 스트림(매개체)을 통하여 읽기기능(Input Steam)으로부터 데이터를 읽어오는 객체
		InputStream in = null;	// <최상위 추상 클래스> 작업방향 : 입력이기때문에 다루는 데이터 단위 : Byte 
		
		byte[] fileData = null;	// 이미지 파일을 읽어오는 저장소
		ResponseEntity<byte[]> entity = null;
		
		// 파일이 존재하지 않을 경우에는 예외가 발생하며 예외처리구문을 던짐.
		try {
			// 파일의 확장자로 파일 종류 확인
			String formatName = fileName.substring(fileName.lastIndexOf(".")+1);
			// 파일명이 이미지의 종류인지 체크
			...
			// 파일 헤더 설정 : 클라이언트인 브라우저에 전송되는 데이터에 대한 설명.
